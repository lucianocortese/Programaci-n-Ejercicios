...
int main()
{
     ...
     char buffer[4][20];//MATRIZ 4 FILAS, una para cada dato y veinte columnas. (REPOSITORIO TRANSITORIO).
     eEmpleado** lista = (eEmpleado**) malloc(sizeof(eEmpleado*)); // LISTA ES ESTATICA QUE APUNTA A UNA VARIABLE DINAMICA.
     eEmpleado** pAuxEmpleado;//AUX ESTATICO
     int cont = 0;
     int cant;

    f = fopen("./empleados.csv","r");//Abrir archivos
    ...
    fscanf(f, "%[^,],%[^,],%[^,],%[^\n]\n", buffer[0], buffer[1], buffer[2], buffer[3]);//PARSER
    ...
    cant = fscanf(f, "%[^,],%[^,],%[^,],%[^\n]\n", buffer[0], buffer[1], buffer[2], buffer[3]);
     ...
     if( cant < 4){
      ...
      //*(lista + cont) = *nuevoEmpleado; ESTO NMO VA MAS.
      *(lista + cont) = nuevoEmpleado;//EN ESTA VERSION VA SOLO NUEVO EMPLEADO PORQUE LE PASAS UNA ESTRUCTURA.
    cont++;
